...
const hasNoWidth = computed(() => {
  return (
    ["str", "dot"].includes(props.element.type) &&
    ["width", "height"].includes(props.field)
  );
});

const charsRegex = computed(() => {
  return props.element.font === "profont22_tr" && props.library === "flipper"
    ? numberFontsRegex
    : standardFontsRegex;
});
...
function onSelect(e) {
  ...
  if (["font"].includes(props.field)) {
    props.element.text = props.element.text
      .replace(charsRegex.value, "")
      .trim();
    updateTextWidth();
  }
  emit("update", props.element);
