...
function canvasMouseDown(e) {
    ...
    if ((e.button !== 0 && !isRightClickDrawing) || isDrawing.value || isMoving.value) {
        return;
    }
    ...
    if ((props.currentLayer && props.currentLayer.type !== 'draw') || props.activeTool === 'select') {
    ...
    if (['draw'].includes(props.activeTool)) {
        const isDrawingCurrent = !!props.currentLayer && props.currentLayer.type === 'draw';
    ...
    } else if (['frame', 'box', 'dot', 'circle', 'disc'].includes(props.activeTool)) {
...
function canvasMouseMove(e) {
    ...
    if (
        ['line', 'frame', 'box', 'circle', 'disc', 'str'].includes(props.activeTool) &&
        e.offsetX >= 0 &&
        e.offsetY >= 0 &&
        ...
        if (props.activeTool === 'frame') {
            x = x >= canvasBoundX.value - scale.value ? canvasBoundX.value - scale.value : x;
            y = y >= canvasBoundY.value - scale.value ? canvasBoundY.value - scale.value : y;
        }
        ...
        if (['str'].includes(props.currentLayer.type)) {
            layerProps.yy = scaleDown(y) - textContainerHeight[props.currentLayer.font];
        }
    ...
    for (let screenElement of screenElements) {
        ...
        switch (type) {
            ...
            case 'line':
                ...
                break;
            case 'circle':
                drawCircle(imgData, x + radius, y + radius, radius, canvasWidth.value, canvasHeight.value);
                ctx.putImageData(imgData, 0, 0);
                break;
            case 'disc':
                drawDisc(imgData, x + radius, y + radius, radius, canvasWidth.value, canvasHeight.value);
                ctx.putImageData(imgData, 0, 0);
                break;
            case 'str':
                ...
                break;
            case 'draw':
                const newImageData = maskAndMixImageData(imgData, props.imageDataCache[screenElement.name], x, y);
                ctx.putImageData(newImageData, 0, 0);
                break;
            default:
                break;
