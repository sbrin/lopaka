...
export function maskAndMixImageData(originalImageData, newImageData, dx, dy) {
  ...
  for (let y = 0; y < originalImageData.height; y++) {
    for (let x = 0; x < originalImageData.width; x++) {
      ...
      if (
        x - dx >= 0 &&
        x - dx < newImageData.width &&
        y - dy >= 0 &&
        y - dy < newImageData.height
      ) {
        // Masking operation
        if (
          newImageData.data[oldPos + 3] <= 127 ||
          newImageData.data[oldPos] +
            newImageData.data[oldPos + 1] +
            newImageData.data[oldPos + 2] >=
            381
        ) {
...
export function addImageDataPadding(
  imageData,
  shiftX,
  shiftY,
  frameWidth,
